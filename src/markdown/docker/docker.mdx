import {CodeHello, DockerIcon, Dockerfile, ImageprogrammerHappy, ImageprogrammerAngry, ImageDockerEngine, Table} from "./docker-cpmponents"
...
จริงเป็นที่มาของคำว่า **มันทำงานได้ในเครื่องผม**
...
1. สร้าง Environment ที่เสถียรและเหมือนกัน
...
### **Container คืออะไร?**
Container เป็นหัวใจสำคัญของ Docker ที่ทำให้การพัฒนาและการ Deploy Application มีประสิทธิภาพและความยืดหยุ่น มาลงรายละเอียดเกี่ยวกับ Container เพื่อเข้าใจให้ลึกขึ้น
...
1. **Development Environment** สร้าง Environment เดียวกันในเครื่องนักพัฒนาและ Production
2. **Microservices** ใช้ Container ในการแยกแต่ละ Service เช่น API, Database, Frontend
3. **CI/CD Pipelines** ใช้ Container สำหรับการ Build และ Test Application แบบอัตโนมัติ
4. **Scaling** ใช้ Container ร่วมกับ [Kubernetes](https://kubernetes.io/) เพื่อ Scale Application ได้อย่างยืดหยุ่น
<div className="border-b"/>
...
<br/>
1. **Dockerfile**
- เราจะสร้าง Image โดยใช้ [Dockerfile](https://docs.docker.com/build/concepts/dockerfile/) ซึ่งระบุขั้นตอนการติดตั้งและตั้งค่า Application
...
3. **Run Container**
    <CodeComparison 
    code={`docker run -p 3000:3000 go-fiber-app`}
    language="bash" 
    filename="Bash"
    lightTheme="github-light"
    darkTheme="github-dark"
...
6. **ขยายเพิ่มเติม**
    ...
    docker stop <container_id> #go-fiber-app

    #ลบ Container ที่ไม่ใช้งาน
    docker rm <container_id> #go-fiber-app
